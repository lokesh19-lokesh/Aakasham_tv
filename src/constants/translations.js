export const TRANSLATION_DICTIONARY = {
  "te": {
    // Telangana Districts
    "Hyderabad": "హైదరాబాద్",
    "Karimnagar": "కరీంనగర్",
    "Nalgonda": "నల్గొండ",
    "Adilabad": "ఆదిలాబాద్",
    "Nizamabad": "నిజామాబాద్",
    "Mahabubnagar": "మహబూబ్ నగర్",
    "Warangal": "వరంగల్",
    "Khammam": "ఖమ్మం",
    "Medak": "మెదక్",
    
    // Andhra Pradesh Districts
    "Kadapa": "కడప",
    "Guntur": "గుంటూరు",
    "Nellore": "నెల్లూరు",
    "Vizianagaram": "విజయనగరం",
    "Anantapur": "అనంతపురం",
    "West Godavari": "పశ్చిమ గోదావరి",
    "East Godavari": "తూర్పు గోదావరి",
    "Kurnool": "కర్నూలు",
    "Srikakulam": "శ్రీకాకుళం",
    "Ongole": "ఒంగోలు",
    "Vizag": "వైజాగ్",
    "Tirupati": "తిరుపతి",
    "Vijayawada": "విజయవాడ",

    // Categories
    "LATEST NEWS": "తాజా వార్తలు",
    "TELANGANA": "తెలంగాణ",
    "ANDHRA PRADESH": "ఆంధ్ర ప్రదేశ్",
    "NATIONAL": "జాతీయ",
    "INTERNATIONAL": "అంతర్జాతీయ",
    "POLITICS": "రాజకీయాలు",
    "CRIME": "నేరం",
    "MOVIE": "సినిమా",
    "LIFESTYLE": "జీవనశైలి",
    "BUSINESS": "వ్యాపారం",
    "TECHNOLOGY": "సాంకేతికత",
    "SPORTS": "క్రీడలు",
    "VIDEOS": "వీడియోలు",
    "MORE": "మరిన్ని",
    
    // More Categories
    "Education": "విద్య",
    "Health": "ఆరోగ్యం",
    "Spiritual": "ఆధ్యాత్మికం",
    "Photos": "ఫోటోలు",
    "E-Paper": "ఈ-పేపర్",
    "Complaints": "ఫిర్యాదులు",
    "Jobs": "ఉద్యోగాలు",
    "Opinion": "అభిప్రాయం",
    "Trending News": "ట్రెండింగ్ వార్తలు",
    "No news articles found for this category.": "ఈ వర్గంలో వార్తా కథనాలు ఏవీ లేవు.",
    "By": "ద్వారా",
    "© 2026 Aakasham TV. All rights reserved.": "© 2026 ఆకాశం టీవీ. అన్ని హక్కులు ప్రత్యేకించబడ్డాయి.",
    "About Us": "మా గురించి",
    "Contact Us": "మమ్మల్ని సంప్రదించండి",
    "Privacy Policy": "గోప్యతా విధానం",
    "Terms of Service": "సేవా నిబంధనలు"
  },
  "hi": {
    // Standard Hindi translations can be added here if needed
  }
};
